...
- `model2vec` (MIT) under `oxn[semantic]` — static embeddings without torch. **Declined 2026-09-17, on the measurement this policy asks for.** The extra was sanctioned here and deferred in P8 until it could be compared against BM25 on the labels P8 built; those labels exist now, and `scripts/measure_semantic.py` runs the comparison. Three models, because the first version of that script tried only `potion-base-8M` — the smallest general-purpose member of the family — and a dependency declined on its weakest candidate is dismissed rather than measured. `potion-retrieval-32M` (retrieval-tuned) and `potion-code-16M-v2` (code-trained) were added, and the fusion constant corrected from k=60 to k=5, which at five-to-forty documents is the difference between weighing two rankings and discarding both. The decline survives all of it: best alone is 0.390 against BM25's 0.610 on the external 100 pairs, best fused is 0.540, and the embedding is wrong in a superset of the places BM25 is wrong there — 11 pairs rescued against 23 damaged, so a hybrid has nothing to recover. On OXN's own 53 the fused code model reaches 0.604 against 0.623, which is parity within one pair, but that is five documents against labels ADR-0006 §5a says ask about *scope* rather than text. Not measured: `all-MiniLM-L6-v2` as a contextual upper bound, since it needs torch and is unshippable here regardless, and no static model came within the 0.05 that would have made the ceiling worth knowing. Sanctioned is not adopted: the entry stays as the record of a dependency considered and measured, which is what this ADR is for.
